...
while (!is_empty(&file)){ 
   Process *mintempex = dequeue(&file);// on fixé que le head de que est le min temps d'escution chaque fois 
    ...
    if (pr.remaining_time > mintempex->remaining_time&& mintempex->remaining_time > 0 && mintempex->ta == currentTime){//si temps a executé du process actuelle superieur a min on inverse et on boucle jusqu'a le file est terminé
        pr=*mintempex;
